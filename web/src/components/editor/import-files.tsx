 ...
 */
export default function ImportFilesButton({ projectId }: { projectId: string }) {
  ...
  const handleFiles = async (list: FileList) => {
    ...
    for (const f of Array.from(list)) {
      ...
      total += f.size;
      if (total > MAX_TOTAL) break;
      files.push({ path: f.webkitRelativePath || f.name, content: await readAsBase64(f) });
    }
    await runImport(files);
  };
    ...
    <>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex items-center gap-1.5 rounded-sm border border-[var(--color-rule)] px-2.5 py-1.5 text-xs text-[var(--color-ink-2)] hover:bg-[var(--color-surface-accent)]"
        >
