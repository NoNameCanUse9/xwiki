...
const projectColumns = `id, name, description, repo_dir, archived_at, created_at, updated_at`

func scanProject(row *sql.Row) (*Project, error) {
	var p Project
	var archivedAt sql.NullString
	var createdAt, updatedAt string
	err := row.Scan(&p.ID, &p.Name, &p.Description, &p.RepoDir, &archivedAt, &createdAt, &updatedAt)
...
func (s *Store) List(ctx context.Context) ([]*Project, error) {
	...
	for rows.Next() {
		...
		var archivedAt sql.NullString
		var createdAt, updatedAt string
		if err := rows.Scan(&p.ID, &p.Name, &p.Description, &p.RepoDir, &archivedAt, &createdAt, &updatedAt); err != nil {
			return nil, err
		}
		if archivedAt.Valid {
			t, err := time.Parse(time.RFC3339, archivedAt.String)
