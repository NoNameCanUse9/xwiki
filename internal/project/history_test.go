...
func TestRevertCreatesNewCommit(t *testing.T) {
	...
	reverted, err := svc.RevertCommit(context.Background(), pid, head, "")
	if err != nil {
	...
	if _, err := svc.RevertCommit(context.Background(), pid, "deadbeefdeadbeefdeadbeefdeadbeefdeadbeef", ""); !errors.Is(err, ErrNotFound) {
		t.Fatalf("unknown sha: want ErrNotFound, got %v", err)
	}
}
