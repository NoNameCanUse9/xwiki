...
function CommitRow({
	...
	const onRevert = async () => {
		if (
			!window.confirm(
				`确认回滚提交 ${shortSha(commit.sha)}？将创建一个新提交。`,
			)
		) {
			return;
		}
			...
			{data && data.stats.length > 0 && (
				<div className="space-y-0.5 pl-1">
					{data.stats.map((s) => (
						<p
							key={s.path}
							className="mono-label !normal-case text-[var(--color-ink-3)]"
						>
							{s.path}{" "}
...
export default function ProjectDetailPage() {
	...
	const commitsQuery = useInfiniteQuery({
		...
		queryFn: ({ pageParam }) => listCommits(id, 5, pageParam, commitSearch),
		initialPageParam: 0,
						...
						<>
							...
							<section className="space-y-4">
								<div className="flex items-center justify-between">
									<p className="mono-label text-[var(--color-ink-3)]">readme</p>
								...
								<input
									...
									value={commitQuery}
									onChange={(event) =>
										setCommitQuery(event.target.value.slice(0, 200))
									}
