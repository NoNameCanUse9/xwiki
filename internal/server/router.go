...
func NewRouter(cfg *config.Config, log *slog.Logger, db *sql.DB, users *user.Store, authSvc *auth.Service, projectsSvc *project.Service, agentSvc *agent.Service, searchSvc *search.Service) http.Handler {
	...
	xh := handlers.NewTransferHandler(cfg, projectsSvc, agentSvc, searchSvc, log)
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Route("/projects", func(r chi.Router) {
			r.Group(func(r chi.Router) {
				...
				r.Use(middleware.SessionAuth(authSvc))
				projectAccess := middleware.AgentProjectAccess(agentSvc)
				r.With(middleware.HumanOnly).Post("/", ph.Create)
				r.With(middleware.HumanOnly).Post("/import-folder", ph.ImportFolder)
				r.Get("/", ph.List)
				r.With(projectAccess).Get("/{id}", ph.Get)
				r.With(middleware.HumanOnly).Patch("/{id}", ph.Rename)
				r.With(middleware.HumanOnly).Delete("/{id}", ph.Delete)
				r.With(middleware.HumanOnly).Post("/{id}/archive", ph.Archive)
				r.With(middleware.HumanOnly).Post("/{id}/unarchive", ph.Unarchive)
				r.With(middleware.HumanOnly).Post("/{id}/restore", ph.RestoreDeleted)
				r.With(middleware.HumanOnly).Delete("/{id}/purge", ph.PurgeDeleted)
				r.With(middleware.HumanOnly).Post("/{id}/purge", ph.Purge)
				r.With(projectAccess).Get("/{id}/docs/tree", dh.Tree)
				r.With(projectAccess).Get("/{id}/docs/home", dh.Home)
				r.With(projectAccess).Get("/{id}/docs/pages/*", dh.Page)
				r.With(projectAccess).Get("/{id}/revision", ch.Revision)
				r.With(projectAccess).Post("/{id}/changesets", ch.Apply)
				r.With(projectAccess).Get("/{id}/locks", lh.Status)
				r.With(projectAccess).Post("/{id}/locks", lh.Acquire)
				r.With(projectAccess).Delete("/{id}/locks", lh.Release)
				r.With(projectAccess).Post("/{id}/locks/heartbeat", lh.Heartbeat)
				r.With(middleware.HumanOnly).Post("/{id}/locks/force-release", lh.ForceRelease)
				r.With(middleware.HumanOnly).Post("/{id}/shares", shareH.Create)
				r.With(projectAccess).Get("/{id}/search", sh.Search)
				r.With(projectAccess).Get("/{id}/backlinks", sh.Backlinks)
				r.With(middleware.HumanOnly).Get("/{id}/export.zip", xh.ExportZip)
				r.With(middleware.HumanOnly).Get("/{id}/export.bundle", xh.ExportBundle)
				r.With(middleware.HumanOnly).Post("/{id}/import", xh.Import)
				r.With(projectAccess).Get("/{id}/attachments/*", ah.Download)
				r.With(middleware.HumanOnly).Get("/{id}/audit", th.Audit)
				r.With(projectAccess).Get("/{id}/commits", hh.Commits)
				r.With(projectAccess).Get("/{id}/commits/{sha}", hh.Commit)
				r.With(projectAccess).Get("/{id}/commits/{sha}/diff", hh.Diff)
				r.With(middleware.HumanOnly).Post("/{id}/commits/{sha}/revert", hh.Revert)
				r.With(projectAccess).Get("/{id}/files/history/*", hh.FileHistory)
			})
		})
	})
