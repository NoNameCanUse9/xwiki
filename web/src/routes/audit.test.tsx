...
describe("AuditPage", () => {
  ...
  it("lists audit entries for the first project", async () => {
    ...
    expect(auditApi.listAudit).toHaveBeenCalledWith("prj_1");
  });
  ...
  it("shows the empty state and supports switching projects", async () => {
    ...
    vi.mocked(auditApi.listAudit).mockResolvedValue({ entries: [] });
    ...
    expect(auditApi.listAudit).toHaveBeenCalledWith("prj_2");
  });
