...
func OpenAPISpec() map[string]any {
	...
	withID("/projects/{id}", "patch", "重命名项目（同步更新 README 标题）")
	withID("/projects/{id}", "delete", "删除项目（元数据 + 仓库，不可恢复）")
	withID("/projects/{id}/archive", "post", "归档项目")
	withID("/projects/{id}/unarchive", "post", "取消归档")
	withID("/projects/{id}/purge", "post", "重写历史彻底删除路径（hard delete，不可恢复）")
